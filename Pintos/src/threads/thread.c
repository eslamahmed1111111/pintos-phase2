               ...
               thread_func *function, void *aux)
{
  ...
  // add child to parent
  // -- create child_info
  struct child*child_info_elem = malloc (sizeof(struct child));
  child_info_elem->tid = child->tid;
  child_info_elem->exit_status = -1;
  child_info_elem->is_loaded = false;
  // -- add child_info to parent list
  list_push_back (&parent->children, &child_info_elem->child_elem);
  }
  /* Add to run queue. */
  thread_unblock (t);
...
struct file_des *
get_file_des (struct thread *t, int fid)
{
  struct list_elem *e = list_begin (&t->files);
  while (e != list_end (&t->files))
  {
    struct file_des *file_des = list_entry (e, struct file_des, thread_elem);
    if (file_des->fid == fid)
      return file_des;
    e = list_next(e);
  }
